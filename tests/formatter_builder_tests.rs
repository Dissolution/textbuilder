...
fn test_append_ascii_char() {
    let string = FormatterBuilder::build_string(|f| f.char('J'));
    assert_eq!(string.as_bytes().len(), 1);
    assert_eq!(string.chars().count(), 1);
    assert_eq!(string.graphemes(false).count(), 1);
    assert_eq!(string, "J");
...
fn test_append_unicode_char() {
    let string = FormatterBuilder::build_string(|f| f.char('💙'));
...
#[test]
fn test_append_str() {
    let string = FormatterBuilder::build_string(|f| f.str(&TextConsts::DIGITS));
...
fn test_append_string() {
    let string = FormatterBuilder::build_string(|f| f.str(&TextConsts::DIGITS.to_string()));
    assert_eq!(string.as_bytes().len(), 10);
    assert_eq!(string.chars().count(), 10);
    assert_eq!(string.graphemes(false).count(), 10);
    assert_eq!(string, "0123456789");
...
fn test_append_debug() {
    ...
    let string = FormatterBuilder::build_string(|f| f.debug(&test));
...
fn test_append_display() {
    ...
    let string = FormatterBuilder::build_string(|f| f.display(&test));
...
#[test]
fn test_args() {
    let string = FormatterBuilder::build_string(|tb| tb.args(format_args!("{:0>8}", 147)));
...
fn test_enumerate() {
    ...
    let string = FormatterBuilder::build_string(|tb| {
...
fn test_append_value() {
    let mut tb = FormatterBuilder::new();
...
fn test_enumerate_append() {
    let mut FormatterBuilder = FormatterBuilder::default();
    let numbers = [0, 1, 2, 3, 4];
    FormatterBuilder.enumerate_append(numbers.iter());
    assert_eq!(FormatterBuilder.len_bytes(), 5);
    assert_eq!(FormatterBuilder.to_string(), "01234")
}
...
fn test_append_delimit() {
    let mut FormatterBuilder = FormatterBuilder::default();
    let numbers = [0, 1, 2, 3, 4];
    FormatterBuilder.append_delimit(',', numbers.iter(), |tb, i, item| {
        tb.append(i.to_string())
            .append(':')
            .append(item.to_string());
    });
    assert!(FormatterBuilder.len_bytes() > 5);
    assert_eq!(FormatterBuilder.to_string(), "0:0,1:1,2:2,3:3,4:4")
}
...
fn test_append_delimit_append() {
    let mut FormatterBuilder = FormatterBuilder::default();
    let numbers = [0, 1, 2, 3, 4];
    FormatterBuilder.append_delimit_append(',', numbers.iter());
    assert_eq!(FormatterBuilder.len_bytes(), 9);
    assert_eq!(FormatterBuilder.to_string(), "0,1,2,3,4")
}
*/
